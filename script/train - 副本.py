...
def valid_loss(model, valid_X, valid_y,valid_y_opi, crf=False):
    ...
    for batch in batch_generator(valid_X, valid_y,valid_y_opi, crf=crf):
        ...
        loss=model(batch_valid_X, batch_valid_X_len, batch_valid_X_mask, batch_valid_y,batch_valid_y_opi)
        losses.append(loss.data[0])
    model.train()
    return sum(losses)/len(losses)

def train(train_X, train_y, train_y_opi, valid_X, valid_y, valid_y_opi, model, model_fn, optimizer, parameters, epochs=200, batch_size=128, crf=False):
    best_loss=float("inf") 
    valid_history=[]
    train_history=[]
    for epoch in range(epochs):
        ...
        for batch in batch_generator(train_X, train_y, train_y_opi, batch_size, crf=crf):
            ...
            loss=model(batch_train_X, batch_train_X_len, batch_train_X_mask, batch_train_y, batch_train_y_opi)
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm(parameters, 1.)
            optimizer.step()
        loss=valid_loss(model, train_X, train_y, train_y_opi, crf=crf)
        print("train_loss : %f" % loss, end='')
        train_history.append(loss)
        loss=valid_loss(model, valid_X, valid_y, train_y_opi, crf=crf)
        print(" || valid_loss : %f" % loss)
...
def run(domain, data_dir, model_dir, valid_split, runs, epochs, lr, dropout, batch_size=128):
    ...
    for r in range(runs):
        ...
        train_history, valid_history=train(train_X, train_y, train_y_opi, valid_X, valid_y, valid_y_opi, model, model_dir+domain+str(r), optimizer, parameters, epochs, crf=False)
